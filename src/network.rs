...
impl Network {
  pub fn new(name: String, n_inp: usize, n_hidden: usize, out_dim: usize, activation: ActivationType, out_activation: ActivationType) -> Network {
  ...
  pub fn from_flatten(weights: Vec<Weight>, name: String, n_inp: usize, n_hidden: usize, out_dim: usize, activation: ActivationType, out_activation: ActivationType) -> Network {
    let mut in_weights = Vec::new();
    let mut hidden_weights = Vec::new();
    let mut out_weights = Vec::new();

    (0..n_inp).into_iter().for_each(|_| {
      in_weights.push(weights.to_owned().pop().unwrap())
    });

    (0..n_inp * n_hidden).into_iter().for_each(|_| {
      hidden_weights.push(weights.to_owned().pop().unwrap())
    });

    (0..out_dim * n_hidden).into_iter().for_each(|_| {
      out_weights.push(weights.to_owned().pop().unwrap())
    });
    ...
    while trained.len() < self.weights.len() {
      ...
      if !trained.contains(&self.weights[ri].to_owned()) {
        ...
        self.weights[ri] = Weight {
          value: self.weights[ri].value - step,
